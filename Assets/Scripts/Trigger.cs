...
public class Trigger : MonoBehaviour
{
    ...
    private bool CanObjectTriggerThis(Collider other)
    {
    ...
    private void OnDrawGizmos()
    {
        ...
        if (drawTriggerArea)
        {
            ...
            if (collider != null && collider is BoxCollider)
            {
                ...
                Vector3 worldCenter = collider.transform.TransformPoint((collider as BoxCollider).center);
                Vector3 worldExtents = collider.transform.TransformVector((collider as BoxCollider).size);
